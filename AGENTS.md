...
- Qbank answer-key integrity audit (2026-08-25): scanned all 4051 `questions` rows for the marked `correct_answer`/`isCorrect` option being explicitly called out as wrong inside its own `explanation` (regex over `(Choice X)` / `(Choices X, Y, and Z)` groups, excluding groups that say "correct"). Found 182 real mismatches (~4.5%) — e.g. id `2cf8ca4b` flagged "Basement membrane" correct while its own explanation argues for "Tendon". 152 were auto-fixable (exactly one option letter never named in any wrong-group → that's the real answer); patched directly via Supabase REST PATCH. 30 left for manual review (`scripts/_qbank-audit/ambiguous-candidates.json`) — either every option got wrong-tagged, multiple untagged candidates, or 7-option questions the A–F letter map didn't cover.
- Separately found 90 rows with no usable answer key at all: 70 have `options: []` outright, 20 have real options but blank `correct_answer`/no `isCorrect:true`. Traced to source: these are `"AI-Generation\n\n"`-prefixed rows already broken in `data/classified-questions.jsonl` (0 options there too) — not a migration bug, a dead-end in the generation pipeline. Naively text-matching them against the jsonl to backfill an answer is unsafe — one attempt pulled a correct-answer string belonging to an unrelated matrix/table sub-question. Fix applied: excluded via `.neq('correct_answer', '')` in `lib/qbank.ts`'s `applyFilters` and in `app/api/quiz-data/route.ts`'s `mode=filters` count/enumeration — bank-visible total dropped from 4051 to 3961. Audit artifacts: `scripts/_qbank-audit/*.json`.
- **`questions` table RLS blocks the anon key entirely** (`select count(*)` via anon returns 0 rows even though service-role sees 4051) — a bare `curl` with no session cookie against `/api/quiz-data` always reads as empty, that's expected, not a regression. Verify quiz-data endpoints either through a real logged-in browser tab, or hit Supabase REST directly with `SUPABASE_SERVICE_ROLE_KEY` from `.env.local`.
- `quiz_live_activity` (Supabase table, written by every `postQuizActivity()` call in `app/quiz/page.tsx`) is a live proxy for "what question is the user on right now" — `select question_number,total_questions,subject,system,is_answered,has_selected_answer,current_question_correct,selected_option_text,question_text order by updated_at desc limit 1` via REST + service-role key answers it with zero browser tooling needed.
- Dev server flakiness: after killing/restarting `next dev` (or after unrelated route edits), a stale `.next/dev/` can leave every route 500ing with `ENOENT ... build-manifest.json` — check `/private/tmp/nextjs-dev.log` (or wherever `next dev` stdout is piped) for that exact error before assuming an app-code regression; `rm -rf .next` + restart clears it (same fix as the existing tsc-phantom-error note above, same root cause: stale `.next`).
<!-- forge-learnings:end -->
